...
Data owner firstly init (s, g:public key), and then use `bilnear accumulator primitive` to calculate digest for each set. 
...
2. the server then release a1 and a0, compute w1, w2 with g and then realse them also.
...
For any column j, client can verify using Acc(cj), The proof size and verification time are both O(1).
