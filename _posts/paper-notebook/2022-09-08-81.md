...
Protocol run in rounds, At the beginning of epoch e:
	...
	1. Each leader proposes a new block extending from the **longest notarized chain** it has seen.
...
- It emphasizes that if there are **two honest consective leaders** after GST, the proposed's hight must grow. $Hight(Bi+1) >=Hight(Bi) + 1$.
- Proof: In epoch e (after GST), one block can be proposed at hight l1. No matter if it conflicts with the current one at l1, the later proposed block must be at length l1+1 or greater.
...
- Proof: The proof follows assume-contradiction method. 

  ```
  Suppose at length of B2 is l2, and B2' is conflicting with B2 => B2' must be proposed before e (Fact3) => B2' parent chain must has length l2-1 and every honest participant has seen it (by Fact1). => At epoch e+1, leader must propose an block at hight greater than l2 => Block proposed at epoch e+1 is smaller than the hight of e+2 ( l2 in other words) => contradiction !
  ```

  Then the B2 cannot have conflict version.  
...
p5 will not vote for it since it is not extended to the longest blockchain, but after p5 receives 2n/3 votes from others, it will still record it as notarized.
...
4. Then from epoch 10, everyting goes normal. 
   ...
   Finally, each participant has B10, B11, B12 all extending from the lonest chain. 
...
In the above example, the protocol become live and can make progress (finalize new blockchain up to B11) after 5 constant epochs (8,9,10,11,12). 

B8 and B9 may has some problem, while from B10 everything goes norm. Finally B10, B11, B12 help to finalize new blockchain.
